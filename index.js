...
const session = require('express-session') // importa express-session (novo)

// configuração do banco de dados do heidi
...
app.post("/login", async (req, res) => {
    ...
    try {
        ...
        if (rows.length > 0) {
            // se o login bem-sucedido, cria a sessão do usuário
...
app.post('/logout', (req, res) => {
    req.session.destroy(err => {
        ...
        res.clearCookie('connect.sid'); // limpa o cookie da sessão novamentte
        res.redirect('/login');
    });
});
...
app.post('/usuarios', async (req, res) => {
    const { usuario, senha } = req.body; // recebe os dados do corpo da requisição (json)
